...
func createRequestManagementServer(
	...
	// namespace := "test"
	// TODO(aaron-prindle) use a fake clock for testing
	clk := clock.RealClock{}

	clientSet := fake.NewSimpleClientset()
	for _, pl := range priorityLevelConfigurations {
		npl, err := clientSet.FlowcontrolV1alpha1().PriorityLevelConfigurations().Create(pl)
		if err != nil {
			panic("oh noo: 1")
		}
		fmt.Printf("npl: %v\n", npl)
	...
	for _, fs := range flowSchemas {
		nfs, err := clientSet.FlowcontrolV1alpha1().FlowSchemas().Create(fs)
		if err != nil {
			panic("oh noo: 2")
		}
		fmt.Printf("nfs: %v\n", nfs)
