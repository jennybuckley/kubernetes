...
func (d SystemPresetData) EnsureSystemPresetConfiguration() genericapiserver.PostStartHookFunc {
	return func(hookContext genericapiserver.PostStartHookContext) error {
		...
		err := wait.Poll(1*time.Second, 30*time.Second, func() (done bool, err error) {
			flowcontrolClientSet, err := flowcontrolclient.NewForConfig(hookContext.LoopbackClientConfig)
			if err != nil {
				...
				if err != nil {
					if apierrors.IsNotFound(err) {
						...
						if err != nil && !apierrors.IsAlreadyExists(err) {
							...
							klog.Infof("created system preset FlowSchema %s", flowSchema.Name)
						}
					} else {
				...
				if err != nil {
					if apierrors.IsNotFound(err) {
						...
						if err != nil && !apierrors.IsAlreadyExists(err) {
							...
							klog.Infof("created system preset PriorityLevelConfiguration %s", priorityLevelConfiguration.Name)
						}
					} else {
						// Unable to get the priority class for reasons other than "not found".
			...
			klog.Infof("all system flow-control settings are created successfully or already exist.")
			return true, nil
		})
