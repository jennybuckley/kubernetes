...
// TestApplyLargeCRD TODO
func TestApplyLargeCRD(t *testing.T) {
	...
	result, err := apiExtensionClient.ApiextensionsV1beta1().RESTClient().Post().SetHeader("Content-Type", "application/yaml").
		AbsPath("/apis", "apiextensions.k8s.io", "v1beta1", "customresourcedefinitions").
		Body(prometheusCRD).
		DoRaw()

	if err != nil {
		t.Fatalf("failed to create custom resource definition in v1beta1: %v:\n%v", err, string(result))
	}

	result, err = apiExtensionClient.ApiextensionsV1beta1().RESTClient().Patch(types.ApplyPatchType).
		AbsPath("/apis", "apiextensions.k8s.io", "v1beta1", "customresourcedefinitions", "prometheuses.monitoring.coreos.com").
		Param("fieldManager", "apply_test").
		Body([]byte(`apiVersion: apiextensions.k8s.io/v1beta1
...
`)).
	...
	if err != nil {
		t.Fatalf("failed to apply to custom resource definition in v1beta1: %v:\n%v", err, string(result))
	}

	result, err = apiExtensionClient.ApiextensionsV1beta1().RESTClient().Patch(types.StrategicMergePatchType).
		AbsPath("/apis", "apiextensions.k8s.io", "v1", "customresourcedefinitions", "prometheuses.monitoring.coreos.com").
		Body([]byte(`{"metadata":{"annotations":{"updated":"test"}}}`)).
		DoRaw()

	if err != nil {
		t.Fatalf("failed to update custom resource definition in v1: %v:\n%v", err, string(result))
	}
}

var prometheusCRD []byte = []byte(`apiVersion: apiextensions.k8s.io/v1beta1
kind: CustomResourceDefinition
metadata:
